...
const arrayUnion = (val) => val;
const collection = () => ({});
const getDocs = async () => ({ docs: [] });

const allowedAdmins = [
  "hetuashar@gmail.com",
  "sahilashar@gmail.com",
  "asharhiten@gmail.com",
  "ronitteli1@gmail.com",
];
...
function AdminPanel() {
  ...
  const fetchUser = async () => {
    ...
    if (userSnap.exists()) {
      ...
      setUserData({ ...data, transactionHistory: sortedTransactions, rechargeHistory: sortedRecharges });
    } else {
  ...
  const updateTransaction = async (index, updatedTx) => {
    ...
    const userRef = doc(db, "users", email);
    await updateDoc(userRef, { transactionHistory: newList });
    fetchUser();
  };
  ...
  const updateRecharge = async (index, updatedRc) => {
    ...
    const userRef = doc(db, "users", email);
    await updateDoc(userRef, { rechargeHistory: newList });
    fetchUser();
  };
